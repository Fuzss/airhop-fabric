    ...
    private static class AirHopHandler extends PacketHandler<C2SAirHopMessage> {
        ...
        private void playEffects(Player player) {
            ...
            if (AirHop.CONFIG.server().hopSound) {
                player.level.playSound(null, player.getX(), player.getY(), player.getZ(), ModRegistry.ENTITY_PLAYER_HOP_SOUND, player.getSoundSource(), 1.0F, 0.6F + player.level.getRandom().nextFloat() * 0.8F);
            }
        }
    }
}
